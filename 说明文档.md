...
>请求路径：/deleteOneRowEmission
>
>请求方式：POST
>
>接口描述：前端传给后端一个source（对应表emissioin_records中的source字段）。后端拿这个字段进行删除

### 3.4.1、请求参数

| 参数名称 | 说明       | 类型   | 是否必须 | 备注 |
| -------- | ---------- | ------ | -------- | ---- |
| source   | 排放源名称 | string | 是       | 无   |
...
>请求方式：POST
>
>接口描述：前端传递给后端一个结构体，对应emission_records表中的字段。

### 3.5.1、请求参数

| 参数名称           | 说明                | 类型   | 是否必须 | 备注 |
| ------------------ | ------------------- | ------ | -------- | ---- |
| dataOrigin         | data_origin         | string | 是       | 无   |
| emissionAmount     | emission_amount     | string | 是       | 无   |
| verificationStatus | verification_status | string | 是       | 无   |
| emissionType       | emissionT_type      | string | 是       | 无   |
| emissionDate       | emission_date       | string | 是       | 无   |
| source             | source              | string | 是       | 无   |
| unit               | unit                | string | 是       | 无   |
...
| code     | number | 必须     |        | 响应标识：1-删除成功，0-删除失败                             |
| msg      | string | 非必须   |        | 删除失败，msg是查询失败的信息                                |
| data     | string | 必须     |        | 返回数据库中插入后的所有数据（同3.2.2 一样。只不过这里是把用户选的删掉了，返回剩下的） |
...
| code     | number | 必须     |        | 响应标识：1-删除成功，0-删除失败                             |
| msg      | string | 非必须   |        | 删除失败，msg是查询失败的信息                                |
| data     | string | 必须     |        | 返回数据库中更新后的所有数据（同3.2.2 一样。只不过这里是把用户选的修改了了，返回修改后所有的的） |
...
>所需数据：data数组，数组项举例：{"text":"北京","value":"北京"},
>
>请求路径：/getDataOrigin
>
>请求方树：GET
>
>接口描述：用于返回所有的dataOrigin字段去重后的信息
...
| 参数名称 | 类型                                                         | 是否必须 | 默认值 | 备注                                                     |
| -------- | ------------------------------------------------------------ | -------- | ------ | -------------------------------------------------------- |
| data     | 一个结构体数组{text：’ ‘，value：’ ‘}，把text的值赋值一份给value | 必须     |        | 返回数据库中所有的dataOrigin字段组成的数组，不可以有重复 |
...
| \|-status           | 项目状态     | String | 非必须   | 已通过<br />待审核<br />已驳回<br />只能为这三者之一，默认为”待审核“ |
...
> 请求路径： /ccer/putProject
>
> 请求方式：POST
>
> 接口描述：用于提供一份项目的信息，将信息添加到所有表单中
...
| 参数名称           | 说明     | 类型   | 是否必须 | 备注                                                         |
| ------------------ | -------- | ------ | -------- | ------------------------------------------------------------ |
| projectName        | 项目名称 | string | 是       | 中文                                                         |
| projectType        | 项目类型 | string | 是       | 可再生能源,<br />林业碳汇,<br />甲烷利用,<br />能源效率提升,<br />碳捕集与封存  <br />只能为这五个其中一之一 |
| location           | 项目地点 | string | 是       | 中文                                                         |
| registrationDate   | 注册日期 | string | 是       | xxxx-xx-xx格式                                               |
| validUntil         | 有效期   | string | 是       | xxxx-xx-xx格式                                               |
| year               | 年度     | number | 是       | XXXX四个数字                                                 |
| reductionAmount    | 减排量   | number | 是       | 实数                                                         |
| verificationDate   | 核查日期 | string | 是       | xxxx-xx-xx格式                                               |
| reportDate         | 报告日期 | string | 是       | xxxx-xx-xx格式                                               |
| reportDetails      | 报告详情 | string | 是       |                                                              |
| registrationNumber | 注册编号 | string | 是       | XXXXDDDDDD四个大写字母6个数字                                |
| issuingAuthority   | 颁发机构 | string | 是       |                                                              |
| issueDate          | 颁发日期 | string | 是       | xxxx-xx-xx格式                                               |
| buyer              | 买方     | string | 是       |                                                              |
| seller             | 卖方     | string | 是       |                                                              |
| transactionDate    | 交易日期 | string | 是       | xxxx-xx-xx格式                                               |
| quantity           | 交易量   | number | 是       | 实数                                                         |
| pricePerUnit       | 单价     | number | 是       | 实数                                                         |
| totalAmount        | 总金额   | number | 是       | 实数                                                         |
...
```js
{
  ...
  projectType: '可再生能源',
  location: '好好',
  registrationDate: "2014-02-09",
  validUntil: "2014-02-09",
  year: 2094,
  reductionAmount: 345.3,
  verificationDate: "2014-02-09",
  reportDate: "2014-02-09",
  reportDetails: '好好好',
  registrationNumber: 'DDDD123123',
  issuingAuthority: 'haohaoha',
  issueDate: "2014-02-09",
  buyer: '我',
  seller: '你',
  transactionDate: "2014-02-09",
  quantity: 23.5,
  pricePerUnit: 1,
  totalAmount: 23.5,
}
...
> 请求路径： /ccer/getProject 
>
> 请求方式：GET
>
> 接口描述：用于将一个项目id的所有信息交出
...
| \|-projectName        | 项目名称 | string | 是       | 中文                                                         |
| \|-projectType        | 项目类型 | string | 是       | 可再生能源,<br />林业碳汇,<br />甲烷利用,<br />能源效率提升,<br />碳捕集与封存  <br />只能为这五个其中一之一 |
| \|-status             | 项目状态 | string | 是       | 已通过<br />待审核<br />已驳回<br />只能为这三者之一，默认为”待审核“ |
| \|-location           | 项目地点 | string | 是       | 中文                                                         |
| \|-registrationDate   | 注册日期 | string | 是       | xxxx-xx-xx格式                                               |
| \|-validUntil         | 有效期   | string | 是       | xxxx-xx-xx格式                                               |
| \|-year               | 年度     | number | 是       | XXXX四个数字                                                 |
...
| status    | 项目状态 | string | 是       | 批准<br />驳回<br />只能为这二者之一 |
